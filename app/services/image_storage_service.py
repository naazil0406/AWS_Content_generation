...
def get_image_bytes(generation_id: str, index: int) -> bytes:
    ...
    key = image_object_key(generation_id, index)
    try:
        obj = _client().get_object(Bucket=settings.GENERATED_IMAGES_BUCKET, Key=key)
        return obj["Body"].read()
    except (ClientError, BotoCoreError) as exc:
        logger.error("Failed to read image %d for %s from S3: %s", index, generation_id, exc)
        raise RuntimeError(f"Generated image not available in S3: {exc}") from exc
